...
- `internal/core/engine/` — worker pool + rate limiter (`x/time/rate`) + orchestration. `BuildJobs` pairs endpoints with applicable checks and enforces the destructive opt-in; `Run` executes them across `max_concurrency` workers drawing from a channel, and sorts results so output does not depend on which worker finished first. The rate limiter is a `ports.HTTPClient` decorator the engine wraps around the client it was given, so it charges per *request*, not per job — a passive check that issues none spends no budget, an active check issuing three is charged three times. Cancelling the context stops new dispatch, lets in-flight jobs finish, and returns partial results with `ctx.Err()`.
