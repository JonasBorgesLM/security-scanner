...
2. **Non-destructive by default, and non-creating by default.** `Endpoint.Destructive` (true for DELETE/PUT/PATCH) means the endpoint is skipped unless `engine.test_destructive` (or a per-endpoint opt-in) is explicitly set. Never add a check or attack that mutates/deletes state without that opt-in gate. **A second gate covers creation:** active checks send a request body only when `engine.test_creates` is set, because a body is what turns a POST probe from something the target refuses at validation into something it stores. The two are separate on purpose — creating is recoverable and deleting is not, so wanting coverage on POST must not force switching on DELETE probing. The engine stamps the setting onto every `Target` as `CanCreate`, so a check held back by it **says so** (`usableParameters` counts what it skipped, `heldBackByCreates` names the setting) rather than declining silently. Read what a route does before switching it on: measured against a five-POST-route spec it is a few hundred created records per scan, and a password-change endpoint probed with a valid body ends the scanner's own session. Baseline collection is stricter still: it only ever sends GET/HEAD/OPTIONS, substituting GET for any unsafe method, because a phase named "collect" must not create or destroy anything.
...
Checks are built in this order (see `doc/security-scanner-projeto.md` §7 and `doc/security-scanner-evolucao.md` §6 for rationale): missing security headers (passive, **done** — `internal/checks/headers.go`) → exposed secrets (passive, **done** — `internal/checks/secrets.go`) → SQLi boolean-based (active, **done** — `internal/checks/sqli.go`) → XSS reflected (active, **done** — `internal/checks/xss.go`) → **auth-required** (active, **done** — `internal/checks/authrequired.go`) → IDOR / weak JWT.

`auth-required` is the one check whose oracle is a **status code** rather than a payload reflecting back, which is why it still reaches a verdict against a target that validates its input — where `sqli-boolean` and `xss-reflected` are refused before reaching anything and can only report `ErrNotExercised`. It uses `Clients.Anonymous`, the only check that should, and it reports only two things without hedging: a 2xx is the finding, a 401/403 is clean. Everything else is an explicit skip naming the status, because a 400 to an unauthenticated request is suggestive (validation normally runs after authentication) and suggestive is not what this project reports on. It sends **no body, ever** — an unauthenticated POST a target actually accepts would create something, and a check hunting a missing control must not depend on exercising it. The two active checks share one request path (`sendProbe` in `internal/checks/probe.go`); a new active check should reuse it rather than re-implement probing.
...
`missing-headers` grades by what the response **is**: a missing `Content-Security-Policy` or `X-Frame-Options` is `medium` on a document and `low` on data, because there is nothing for a policy to restrict or for a user to click in a JSON body. It **downgrades rather than suppresses** — OWASP recommends all four on API responses, and staying silent would make "no CSP finding" mean either "it is set" or "we decided not to look", which is invariant 6's ambiguity one level down. `X-Content-Type-Options` is never downgraded: sniffing a JSON body into HTML is the exact attack it prevents. An unrecognised or absent `Content-Type` counts as a document, since over-reporting a low-severity finding costs attention and quietly demoting a real one costs the finding.
