...
var recommendations = map[string]string{
	...
	"cache-on-authenticated": "Send `Cache-Control: no-store` on responses that require authentication. It is the only directive that keeps the body out of storage — `no-cache` means \"revalidate before reuse\" and still writes it to disk, and `private` only keeps it out of SHARED caches. Set it in the middleware that already handles auth, so a new protected route cannot ship without it.",
	"cors-misconfigured":     "Replace the reflected origin with an explicit allow-list of the origins that genuinely need access, and keep Access-Control-Allow-Credentials off unless one of them does. Echoing the request's Origin back is the defect: it turns \"who may read this\" into \"whoever asked\". If a reflection must stay, send Vary: Origin so a shared cache cannot hand one origin's answer to another.",
	"dangerous-http-methods": "Disable TRACE at the web server or proxy that terminates the connection — it is a one-line directive in nginx, Apache and most reverse proxies, and no API needs it. The urgency is low: the Cross-Site Tracing attack this is traditionally reported for stopped being reachable when browsers refused to let script issue TRACE. What remains is that the echo discloses headers a proxy chain added.",
	"jwt-weak":               "For alg:none: reject any token whose header algorithm is not one you explicitly allow, and never let the header choose the verification algorithm — decide it server-side. For a long-lived token: shorten exp and pair it with a refresh mechanism, so a leaked token expires before it is useful. Both are one-line policy in every mainstream JWT library.",
	"idor":                   "Check ownership in the handler, not just authentication in the middleware: a valid session says who is asking, never what they may see. Load the object, compare its owner against the caller, and refuse otherwise — returning 404 rather than 403 is a reasonable choice, since it does not confirm the resource exists.",
	"verbose-errors":         "Return a generic error envelope to clients and log the detail server-side. A stack trace, SQL error or filesystem path in a response hands an attacker a map of your internals; the fix is a recover-and-sanitise layer at the edge, and it belongs on every route rather than the one this finding names.",
	"open-redirect":          "Do not put a user-supplied value into a Location header. Redirect to a fixed path, or map the parameter through an allow-list of permitted destinations and refuse anything else — never redirect to a raw URL from the request, even one that looks internal, since // and \\ tricks turn a relative-looking value absolute.",
